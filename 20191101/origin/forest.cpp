...
long long n;
int m, q, op; long long out = 0;
long long ans = 0;
struct home{
	int cnt, size, k, r;
	long long cap, ful;
	home(){}
	home(int cnt, int size):cnt(cnt),size(size),k(0),r(0),cap((long long)size*cnt),ful(cap){}
	long long operator ()() {if(~size) return (long long)k * size * size + r * r; else return out;}
	long long add(long long num) {
		if(num <= cap){
			cap -= num; k = (ful - cap) / size; r = (ful - cap) % size;
			return 0;
		}
	...
	bool operator > (const home &b) {return size > b.size;}
}homes[400020];

int nxt[400020], pre[400020]; int tail, head;
...
void init(int op, int m){
	...
	long long cap = n;
	if(m > 0) {cap = homes[start[0]].add(cap); ans += homes[start[0]]();}
	int i = 1;
	while(cap && i < m){
		cap = homes[start[i]].add(cap); ans += homes[start[i]](); i++;
	}
	pre[start[0]] = -1;
	for(int j = 1; j < m; j++) 
	...
	if(op){
		int top = m + q;
		sort(rnk, rnk + top, cmp);
		pre[rnk[0]] = -1;
		for(int i = 1; i < top; i++)
			if(rnk[i] >= m){int k = i - 1;for(;k&&rnk[k]<rnk[i]; k--);pre[rnk[i]] = rnk[k];}
		nxt[rnk[top-1]] = -1;
		for(int i = top - 2; i >= 0; i--) 
			if(rnk[i] >= m){int k = i + 1;for(;k<top-1&&rnk[k]>rnk[i];k++); nxt[rnk[i]] = rnk[i];}
		head = m - 1;
	}
}

void add(int x){
	if(pre[x] != m + q && pre[x] != -1) nxt[pre[x]] = x; 
	if(nxt[x]!=-1 && nxt[x] != m + q) pre[nxt[x]] = x; 
	if(head == -1) head = x;
	while(homes[x].cap && (tail == -1 || rnk[tail] > rnk[x])){
		if(tail == -1){
			long long cut = out;
			cut -= (out = homes[x].add(out));
			ans -= cut;
			ans += (homes[x]());
			if(out == 0) tail = head, head = pre[head] ;
		}
		else {
			ans -= homes[x]();
			ans -= homes[tail](); 
			...
			ans += homes[x]();
			ans += homes[tail]();
			if(homes[tail].ful == homes[tail].cap) tail = head, head = pre[head];
		}
	}
	printf("%lld\n", ans);
}
